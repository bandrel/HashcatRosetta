    ...
    while i < len(rule_str):
        ...
        # Handle parameterized rules
            ...
            try:
                ...
                steps.append(f"i{pos_char}{val_char}: Insert '{val_char}' at pos {pos} → {prev} → {current}")
                i += 3
...
@click.option('--baseword', type=str, default='password', help='Baseword to transform (used with --explain, default: password)')
...
@click.option('--metric', type=click.Choice(['frequency', 'basewords', 'candidates']), 
              default='frequency', help='Metric to rank rules by (used with --rules)')
...
def main(ctx, file, explain, baseword, rules, basewords, export, metric, format, top, min_occurrences, detail):
    ...
    if not rules and not basewords and not export:
        ...
        click.echo(f"      Average per Baseword: {bw_stats.get('avg_occurrences_per_baseword', 0):.2f}")
    ...
    if basewords:
        ...
        for baseword, count in baseword_list:
            ...
            if detail:
                ...
                click.echo(f"  Rules Applied: {', '.join(sorted(set(occ['rule'] for occ in bw_detail['occurrences'])))}")
...
def _export_to_csv(analyzer, filepath):
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        for rule in sorted(analyzer.rule_stats.keys()):
            ...
            writer.writerow([rule, stats['count'], len(stats['basewords']), len(stats['candidates'])])
