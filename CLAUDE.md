...
- Tests marked with `@pytest.mark.integration` require an external resource not present in CI (the hashcat binary, `generate-rules.bin`, or a machine-local rule corpus supplied via `$HASHCAT_ROSETTA_RULE_CORPUS`)
